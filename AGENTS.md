...
- **Comments**: When using `bun` for file tasks, include MISE attributes as comments starting with `//MISE` (no space between `//` and `MISE`). For example, `//MISE description="Task description"`. You should also include `//USAGE` or `// USAGE` comments to provide examples if appropriate.
